...
Herald is a fork of [agent-notify](https://github.com/LetTTGACO/agent-notify)
extended with a durable execution monitor. The upstream notify pipeline is
kept intact; everything Herald adds sits behind it and is failure-isolated.
...
```
 ...
 │                                             │   │    ▼                                        │
 │  herald-host (LaunchAgent, 30 s tick)          │   │  MonitorHub ── state ──▶ SQLite (/data)     │
...
| Host Bridge | `host/herald-host.mjs` | Thin macOS facts collector (LaunchAgent): process presence, active rollout/session UUIDs (Codex `sessions/` and Qoder `projects/<slug>/`), sleep-gap. No policy, no notification decisions. The Qoder desktop binary is a resident marker — only its transcripts and its CLI process count as work. |
...
- **One path to the phone for monitor logic**: everything the monitor decides
  ...
  report cooperatively via `emit` (`./herald emit …` or HTTP).
...
- Claude Code/CI workers: the cooperative `emit` contract needs no Herald-side code
